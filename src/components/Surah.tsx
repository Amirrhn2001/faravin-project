...
import { quran } from "../assets/quran";
import translate from '../redux/exportSurahTranslate';
...
const Surah = (props: {}) => {
  ...
  const [translateSrc, setTranslateSrc] = useState<string>()
  const [aye, setAye] = useState<number>(1)
  const [isPlayed, setIsPlayed] = useState<boolean>(false)
  const audioRef = useRef<HTMLAudioElement>(null)
  const translateRef = useRef<HTMLAudioElement>(null)
  if (id) surah = +id;
  const handleStart = async(i: number) => { 
    const ayeB = `${i}`.padStart(3,'0')
    const surahB = `${surah + 1}`.padStart(3,'0')
  ...
  return (
    <div className='bg-isabelline w-2/5 h-fit'>
      ...
      <div className='sticky z-20 bottom-0 flex justify-center items-center mx-auto box-border bg-verdigris p-5 rounded-tl-3xl rounded-tr-3xl'>
        <ul className='flex list-none justify-center items-center gap-5 '>
          <li onClick={() => isPlayed ? handlePause() : handleStart(aye)} className='cursor-pointer'>{isPlayed ? <TbPlayerPause color='white' size={30}/> : <TbPlayerPlay color='white' size={30}/>}</li>
      ...
      <audio src={translateSrc} ref={translateRef} onPlay={handleStartTranslate} onEnded={handleEndTranslate}></audio>
    </div>
  )
}
